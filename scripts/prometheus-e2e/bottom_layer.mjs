...
export function sampleShmDirect(containerName = 'aurora-node') {
  ...
  return {
    ...
    waf: {
      ...
      blocked: r64(144),
      audit: r64(152),
      evaluations: r64(136) + r64(144) + r64(152),
    },
    access: {
      allowed: r64(160),
      blocked: r64(168),
    ...
    conn_limit_rejected: r64(232),
    traffic_shaper_delayed: r64(240),
    request_size_rejected: r64(248),
    termination_triggered: r64(256),
    traffic_split_primary: r64(264),
    traffic_split_secondary: r64(272),
    canary_baseline: r64(280),
    canary_canary: r64(288),
    blue_green_blue: r64(296),
    blue_green_green: r64(304),
    mirror_sampled: r64(312),
    connections: {
      active: r64(320),
      reading: r64(328),
      writing: r64(336),
...
/**
 * Asserts memory boundedness (no memory leaks).
 */
export function assertMemoryBounded(before, after, maxAllowedGrowthPercent = 25) {
  if (before.currentBytes === 0 || after.currentBytes === 0) return;
  const growthBytes = after.currentBytes - before.currentBytes;
  const growthPct = (growthBytes / before.currentBytes) * 100;
  console.log(`[Memory Audit] Before: ${before.currentMB} MB, After: ${after.currentMB} MB (Delta: ${growthPct.toFixed(2)}%)`);
  assert.ok(
    growthPct <= maxAllowedGrowthPercent,
    `Memory leak detected! Working set grew by ${growthPct.toFixed(1)}% (max allowed: ${maxAllowedGrowthPercent}%)`
  );
}
