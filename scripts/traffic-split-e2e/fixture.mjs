...
export class TrafficSplitFixture {
  constructor(root, options = {}) {
    this.root = root;
    this.options = options;
    this.dir = mkdtempSync(path.join(os.tmpdir(), 'aurora-traffic-split-e2e-'));
    ...
    this.upstreams = {
      ...
      backend_v4: { port: 0, server: null, count: 0 },
    };
  ...
  async startControlPlane() {
    ...
    this.controlServer = http.createServer((req, res) => {
      ...
      if (req.method === 'GET' && url.pathname === '/api/v1/traffic-split') {
      ...
      if (req.method === 'POST' && url.pathname === '/api/v1/traffic-split') {
        let bodyStr = '';
        ...
        req.on('end', () => {
          try {
            ...
            if (Array.isArray(payload.rules)) {
              ...
              rules = [{
                id: payload.id || `rule-gen-${this.generation + 1}`,
                priority: payload.priority || 10,
                origin: payload.origin || '*',
                path_prefix: payload.path_prefix || '/',
                split_by: payload.split_by || 'random',
                header_name: payload.header_name || 'x-user-id',
      ...
      if (req.method === 'POST' && url.pathname === '/api/v1/stats/reset') {
  ...
  async mutatePolicyViaApi(body) {
    const res = await fetch(`http://127.0.0.1:${this.controlPort}/api/v1/traffic-split`, {
      method: 'POST',
  ...
  async resetStatsViaApi() {
    const res = await fetch(`http://127.0.0.1:${this.controlPort}/api/v1/stats/reset`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    });
    return await res.json();
  ...
  async getStatsViaApi() {
    const res = await fetch(`http://127.0.0.1:${this.controlPort}/api/v1/stats`);
    return await res.json();
  }
...
http {
    ...
    upstream backend_v1 {
        server 127.0.0.1:${this.upstreams.backend_v1.port};
    }
    upstream backend_v2 {
        server 127.0.0.1:${this.upstreams.backend_v2.port};
    }
    upstream backend_v3 {
        server 127.0.0.1:${this.upstreams.backend_v3.port};
    }
    upstream backend_v4 {
        server 127.0.0.1:${this.upstreams.backend_v4.port};
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = http.request({
        ...
        res.on('end', () => {
          ...
          const servedBy = res.headers['x-served-by'] || res.headers['x-aurora-upstream'] || null;
          let json = null;
          try {
