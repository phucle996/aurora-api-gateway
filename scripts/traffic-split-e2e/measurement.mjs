...
export class TrafficSplitMeasurement {
  constructor() {
    this.count = 0;
    this.sumLatencyMs = 0;
  ...
  record(statusCode, latencyMs, upstream = null, error = null) {
    const now = Date.now();
    this.count++;
  ...
  finish() {
    ...
    for (const [up, cnt] of Object.entries(this.upstreams)) {
      percentages[up] = parseFloat(((cnt / this.count) * 100).toFixed(2));
    }
    ...
    return {
      ...
      upstreamPercentages: percentages,
      latencyMs: {
