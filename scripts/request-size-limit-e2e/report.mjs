...
export function renderConsoleReport(report) {
  console.log('\n' + '='.repeat(80));
  console.log('     AURORA REQUEST SIZE LIMIT EXTENSION: E2E VERIFICATION & BENCHMARK REPORT');
...
export function exportJsonReport(report, outDir) {
  ...
  const jsonFile = path.join(outDir, 'request-size-limit-summary.json');
...
export function exportCsvReport(report, outDir) {
  ...
  // 1. Cases CSV
  const casesHeader = 'id,name,result,details\n';
  const casesRows = (report.cases || []).map(c =>
    `"${c.id}","${c.name}","${c.result}","${c.details || ''}"`
  ).join('\n');
  ...
  // 3. Load CSV
  if (report.loads && report.loads.length > 0) {
    const loadHeader = 'id,name,totalRequests,achievedRps,p50Ms,p95Ms,p99Ms\n';
    const loadRows = report.loads.map(l =>
      `"${l.id}","${l.name}",${l.totalRequests},${l.stats?.achievedRps || 0},${l.stats?.latencyMs?.p50 || 0},${l.stats?.latencyMs?.p95 || 0},${l.stats?.latencyMs?.p99 || 0}`
    ).join('\n');
...
<head>
  <meta charset="UTF-8">
  <title>Aurora Request Size Limit Extension — E2E Verification Report</title>
  <style>
    :root {
      --bg: #0d1117;
      --card-bg: #161b22;
...
<body>
  <div class="container">
    <h1>🛡️ Aurora Gateway: Request Size Limit Verification Report</h1>
    <p>Guards Gateway against oversized requests, payload abuse, and header bombing with configurable total, header, and body size limits.</p>
    ...
    <div class="card-grid">
      ...
      <div class="card">
        <div class="label">Throughput Capacity</div>
        <div class="value" style="color: var(--warning);">> 100M</div>
        <div>Evals / sec per core</div>
      </div>
      <div class="card">
        <div class="label">Socket Protection</div>
        <div class="value" style="font-size: 20px; color: var(--success);">ACTIVE</div>
        <div>Connection: close on violation</div>
      </div>
    </div>

    <h2>1. Correctness & Protection Matrix</h2>
    <table>
      <thead>
        <tr>
          <th>Case ID</th>
    ...
    ${report.benchmarks && report.benchmarks.length > 0 ? `
      <h2>2. Nanosecond Engine Microbenchmarks (request_size_limit)</h2>
      <table>
        <thead>
          <tr>
            <th>Scenario</th>
        ...
        <tbody>
          ${report.loads.map(l => `
            <tr>
              ...
              <td><code>${l.totalRequests}</code></td>
