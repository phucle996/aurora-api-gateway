export class CanaryReleaseMeasurement {
  constructor() {
    this.count = 0;
    this.sumLatencyMs = 0;
  ...
  record(statusCode, latencyMs, upstream = null, canaryStatus = null, error = null) {
    const now = Date.now();
    this.count++;
    ...
    if (canaryStatus) {
      this.canaryStatuses[canaryStatus] = (this.canaryStatuses[canaryStatus] || 0) + 1;
    }
    if (error) {
  ...
  finish() {
    if (this.count === 0) {
      return {
        ...
        upstreamPercentages: {},
        canaryStatusDistribution: {},
        canaryStatusPercentages: {},
    ...
    const canaryStatusPercentages = {};
    for (const [st, cnt] of Object.entries(this.canaryStatuses)) {
      canaryStatusPercentages[st] = parseFloat(((cnt / this.count) * 100).toFixed(2));
    }
    ...
    return {
      ...
      upstreamPercentages,
      canaryStatusDistribution: this.canaryStatuses,
      canaryStatusPercentages,
      latencyMs: {
