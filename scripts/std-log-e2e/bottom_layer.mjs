...
export function assertMemoryBounded(before, after, maxGrowthMB = 25) {
  const beforeMB = typeof before === 'object' ? Number(before.currentMB) : Number(before);
  const afterMB = typeof after === 'object' ? Number(after.currentMB) : Number(after);
  const delta = Math.abs(afterMB - beforeMB);
  assert.ok(
    delta <= maxGrowthMB,
    `Memory leak detected: memory delta ${delta.toFixed(2)} MB exceeded threshold ${maxGrowthMB} MB`
  );
  return delta;
}
