...
export async function runRateLimitUiJourney(lab) {
  ...
  const page = await context.newPage(); lab.page = page;
  ...
  lab.report.ui.push({ action: 'real-login', durationMs: performance.now() - start, authentication: 'UI password form; no localStorage injection' });

  await page.goto(lab.base + '/upstreams/create');
  await page.getByPlaceholder('e.g. prod-api-cluster').fill('rate_limit_origin');
  ...
  const created = await upstreamResponse; assert.equal(created.status(), 201, await created.text());
  ...
  for (const host of ['rate-limit.test', 'other-rate-limit.test']) {
    ...
    await page.locator('form select').filter({ has: page.locator('option[value="rate_limit_origin"]') }).selectOption('rate_limit_origin');
    ...
    const response = await routeResponse; assert.equal(response.status(), 201, await response.text());
  ...
  for (const node of lab.nodes) {
    await lab.wait('UI route served by ' + node.id, async () => {
      const response = await lab.probe(node, '/ui-origin');
      return response.status === 200 && response.headers['x-lab-origin'] === 'true';
    });
  }
  await page.screenshot({ path: path.join(lab.dir, 'ui-routes.png'), fullPage: true });
}
