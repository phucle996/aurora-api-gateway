...
try{
 for(const id of ['node-03','node-01','node-02']){const n=await waitNode(id,n=>n.fresh&&n.installable&&n.modules.some(m=>m.name==='gzip'&&m.loaded));assert.ok(n.modules.some(m=>m.name==='http2_upstream'&&m.available));assert.ok(n.modules.some(m=>m.name==='ngx_http_aurora_waf_module'));}
