...
export class BlueGreenFixture {
  constructor(root, options = {}) {
    this.root = root;
    this.options = options;
    this.dir = mkdtempSync(path.join(os.tmpdir(), 'aurora-blue-green-e2e-'));
    ['client', 'proxy', 'fastcgi', 'uwsgi', 'scgi'].forEach(d =>
      mkdirSync(path.join(this.dir, d), { recursive: true })
    );

    this.blueGreenPolicyPath = path.join(this.dir, 'active-blue-green.json');
    ...
    this.upstreams = {
      app_blue: { port: 0, server: null, count: 0 },
      app_green: { port: 0, server: null, count: 0 },
  ...
  writePolicy(config) {
    ...
    writeFileSync(this.blueGreenPolicyPath, JSON.stringify(snapshot, null, 2));
  }
  ...
  async startControlPlane() {
    ...
    this.controlServer = http.createServer((req, res) => {
      ...
      if (req.method === 'GET' && url.pathname === '/api/blue-green') {
      ...
      if (req.method === 'POST' && url.pathname === '/api/blue-green') {
        let bodyStr = '';
        ...
        req.on('end', () => {
          try {
            ...
            } else if (payload.blue_upstream && payload.green_upstream) {
              policyData = {
                active_slot: payload.active_slot || 'blue',
                blue_upstream: payload.blue_upstream,
                green_upstream: payload.green_upstream,
                switch_header: payload.switch_header || 'x-deploy-slot',
              };
            } else {
              policyData.rules = [{
                id: payload.id || `bg-rule-${this.generation + 1}`,
                priority: payload.priority || 10,
                origin: payload.origin || '*',
                path_prefix: payload.path_prefix || '/',
                active_slot: payload.active_slot || 'blue',
                blue_upstream: payload.blue_upstream || 'app_blue',
                green_upstream: payload.green_upstream || 'app_green',
                switch_header: payload.switch_header || 'x-deploy-slot',
                blue_upstream_headers: payload.blue_upstream_headers || [],
                green_upstream_headers: payload.green_upstream_headers || [],
              }];
            }
  ...
  async mutatePolicyViaApi(body) {
    const res = await fetch(`http://127.0.0.1:${this.controlPort}/api/blue-green`, {
      method: 'POST',
...
http {
    ...
    upstream app_blue {
        server 127.0.0.1:${this.upstreams.app_blue.port};
    }
    upstream app_green {
        server 127.0.0.1:${this.upstreams.app_green.port};
    }
    ...
    server {
        ...
        server_name localhost example.com bluegreen.example.com other.example.com;

        gateway on;
        gateway_waf_policy ${this.wafPolicyPath};
        gateway_blue_green_policy ${this.blueGreenPolicyPath};
        ...
        location / {
            ...
            add_header X-Aurora-Deploy-Slot $gateway_deploy_slot always;
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = http.request({
        ...
        res.on('end', () => {
          ...
          const deploySlot = res.headers['x-aurora-deploy-slot'] || null;
          let json = null;
          try {
