...
// A JWT-specific accumulator: normalizes generator latency nanoseconds to milliseconds.
// Histogram quantiles have at most 0.1% bucket rounding; min/max/mean and counters are exact.
export class JwtMeasurement {
  constructor() {
  ...
  finish() {
    ...
    return {
      requests: this.count,
      elapsedSeconds,
      achievedRps: elapsedSeconds > 0 ? this.count / elapsedSeconds : null,
      latencyMs: {
        ...
        quantileRelativeRounding: 0.001
      },
      ...
      series: [...this.seconds.values()].map(p => ({ ...p, meanMs: p.latencySumMs / p.requests })),
      worst: this.worst
    };
  }
}
