...
export function RulesStats({ stats }: { stats: RulesStatsData | null }) {
  ...
  return (
    <section aria-label="Global rule statistics" className="space-y-2">
      <p className="text-xs text-slate-500 dark:text-slate-400">All saved rules · independent of table filters · {stats ? `Read at ${stats.as_of} · comparison: end of previous calendar month (UTC)` : 'Statistics unavailable while loading or after a failed request.'}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {cards.map(({label, count, delta}) => (
          <div key={label} aria-label={label} className="bg-white dark:bg-[#0B1320] border border-slate-200 dark:border-[#172338] p-4 font-sans shadow-xs rounded-sm transition-colors">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{label}</p>
            <p className="text-2xl text-slate-900 dark:text-white mt-1.5 font-bold tracking-tight">{count ?? '—'}</p>
            <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-2 font-normal" title={stats ? `Baseline: latest revision strictly before ${stats.comparison_before}` : undefined}>
