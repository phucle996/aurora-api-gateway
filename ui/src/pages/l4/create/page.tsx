...
export type L4TrafficAction = 'forward' | 'deny' | 'continue';
...
export default function CreateL4ServicePage() {
  ...
  const handleSelectPreset = (preset: { label: string; port: number; proto: 'tcp' | 'udp' }) => {
    ...
    if (!name.trim()) {
      setName(`${preset.label.toLowerCase()}-stream`);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Build payload based on 3 options: forward to / deny / continue
          ...
          <div className="col-span-12 lg:col-span-8 space-y-6">
            ...
            <div className="p-5 bg-card border border-border rounded-xl space-y-4 shadow-xs backdrop-blur-xs">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                >
                  Enable L4 stream listener immediately upon creation
                </label>
              </div>
            </div>
