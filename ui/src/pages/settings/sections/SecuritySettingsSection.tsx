        ...
        <div className="mt-3 space-y-3 text-xs font-mono">
          ...
          <div className="flex items-center justify-between py-1 border-b border-[#152030]/60">
            ...
            <button
              ...
              className="px-2.5 py-1 bg-[#0E1726] hover:bg-[#152030] border border-[#1C293D] text-slate-200 text-xs transition-colors cursor-pointer"
            >
              Configure
            </button>
          </div>
          ...
          <div className="pt-2 space-y-2.5">
            ...
            {/* Enable API Keys */}
            <div className="flex items-center justify-between">
              <span className="text-slate-400">Enable API Keys</span>
              <button
                type="button"
                onClick={() => setEnableApiKeys(!enableApiKeys)}
                className={`w-9 h-5 flex items-center p-0.5 cursor-pointer transition-colors ${
                  enableApiKeys ? 'bg-emerald-600' : 'bg-[#152030]'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white transition-transform ${
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-400">Allow from Internal Network Only</span>
              <button
                type="button"
                onClick={() => setInternalOnly(!internalOnly)}
                className={`w-9 h-5 flex items-center p-0.5 cursor-pointer transition-colors ${
                  internalOnly ? 'bg-emerald-600' : 'bg-[#152030]'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white transition-transform ${
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-400">Audit Log for Admin Actions</span>
              <button
                type="button"
                onClick={() => setAuditLogAdmin(!auditLogAdmin)}
                className={`w-9 h-5 flex items-center p-0.5 cursor-pointer transition-colors ${
                  auditLogAdmin ? 'bg-emerald-600' : 'bg-[#152030]'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white transition-transform ${
