...
export default function EditDomainPage() {
 ...
 useEffect(() => {
  ...
  void (async () => {
   try {
    ...
    const d = await domainsApi.get(id);
    if (active) { setSaved(d); setDomain(d.domain); setTarget(d.upstream); setStatus(d.status); setDescription(d.description); setTags(d.tags.join(', ')); }
 ...
 const submit = async (e: FormEvent) => {
  ...
  try {
   ...
   if (!id || !saved) throw new Error('Domain has not loaded');
   await domainsApi.update(id, {...payload, tls_type: saved.tlsType, min_tls_version: saved.minTlsVersion,
    hsts_enabled: saved.hstsEnabled, ocsp_stapling: saved.ocspStapling, client_ca_subject: saved.clientCaSubject,
    upstream_algorithm: saved.upstreamAlgorithm, health_check_path: saved.healthCheckPath});
   navigate('/domains');
 ...
 return <div className="p-6 max-w-3xl mx-auto space-y-5">
  ...
  <h1 className="text-2xl font-semibold">Save Domain</h1>
  ...
  <form onSubmit={submit}>
   <fieldset disabled={loading || busy || !saved} className="space-y-5 disabled:opacity-60">
    <label className="block text-sm space-y-2">Domain hostname
     <input required readOnly={true} value={domain} onChange={e => setDomain(e.target.value)} placeholder="app.example.com" className="block w-full p-2 border border-input rounded bg-background" />
    </label>
    ...
    <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground rounded">{busy ? 'Saving…' : 'Save Domain'}</button>
   </fieldset>
  </form>
 </div>;
}
