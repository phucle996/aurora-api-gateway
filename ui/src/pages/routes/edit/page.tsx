...
import { routesApi, type RouteItem } from '../../../lib/api/routes';
...
export default function EditRoutePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      routesApi.get(id).catch(async () => {
        // Fallback to finding in list if GET /id is not directly supported
        const res = await routesApi.list({ limit: 200 });
        const item = (res.items || []).find((r) => r.id === id);
        if (!item) throw new Error(`Route with ID "${id}" not found.`);
        return item;
      }),
      upstreamsApi.list({ limit: 100 }),
    ])
      .then(([routeData, upstreamsRes]) => {
        setOriginalRoute(routeData);
        setFormData({
          name: routeData.name,
          host: routeData.host,
          path: routeData.path,
          upstream_name: routeData.upstream_name,
          enabled: routeData.enabled,
          strip_path: routeData.strip_path,
          websocket: routeData.websocket,
          priority: routeData.priority,
          plugins_json: routeData.plugins_json || '{}',
          description: routeData.description || '',
        });

        const names = (upstreamsRes.items || []).map((u) => u.name);
        setUpstreams(names);

        if (routeData.plugins_json && routeData.plugins_json.trim() !== '{}' && routeData.plugins_json.trim() !== '') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.plugins_json && formData.plugins_json.trim() !== '') {
      try {
        ...
        setError('Plugins JSON is invalid. Please fix JSON syntax.');
        return;
      }
    }
  ...
  if (loading) {
    return (
      <div className="p-12 max-w-5xl mx-auto flex flex-col items-center justify-center space-y-3 font-sans">
        <div className="w-8 h-8 rounded-full border-2 border-primary border-t-transparent animate-spin" />
        <span className="text-xs text-muted-foreground font-mono">Loading route configuration...</span>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6 font-sans">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span>
          <span className="text-foreground font-medium">Edit Route</span>
          <span>/</span>
          <span className="font-mono text-xs text-primary">{originalRoute?.name || id}</span>
        </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold tracking-tight text-foreground">
                  Edit Route: {originalRoute?.name}
                </h1>
                <span className="font-mono text-[11px] bg-muted px-2 py-0.5 rounded-sm text-muted-foreground">
                  ID: {id}
                </span>
              </div>
              <p className="text-xs text-muted-foreground mt-0.5">
                Update traffic matching rules, protocol policies, or target upstream destination.
              </p>
            </div>
          </div>
