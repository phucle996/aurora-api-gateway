...
func TestSQLiteRestartAndConnectionSettings(t *testing.T) {
	...
	if err := db.QueryRowContext(ctx, "SELECT COUNT(*) FROM extension_instances").Scan(&extensionInstanceCount); err != nil || extensionInstanceCount != 19 {
		t.Fatalf("expected 19 packaged extension instances, got count = %d, err = %v", extensionInstanceCount, err)
	}
