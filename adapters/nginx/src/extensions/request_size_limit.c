...
static void ngx_http_gateway_request_size_limit_cleanup(void *data) {
...
char *ngx_http_gateway_merge_request_size_limit(ngx_conf_t *cf,
  ...
  ngx_conf_merge_str_value(conf->request_size_limit_policy,
                           prev->request_size_limit_policy, "");
  if (conf->request_size_limit_policy.len == 0) {
    return NGX_CONF_OK;
  }

  if (prev->request_size_limit_engine &&
      conf->request_size_limit_policy.data ==
          prev->request_size_limit_policy.data) {
    conf->request_size_limit_engine = prev->request_size_limit_engine;
    return NGX_CONF_OK;
  }
  if (ngx_conf_full_name(cf->cycle, &conf->request_size_limit_policy, 0) !=
      NGX_OK) {
    return NGX_CONF_ERROR;
  }

  fd =
      open((char *)conf->request_size_limit_policy.data, O_RDONLY | O_NONBLOCK);
  if (fd == -1) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
                       "cannot open Gateway request size limit policy %V",
                       &conf->request_size_limit_policy);
    return NGX_CONF_ERROR;
  }
  if (fstat(fd, &st) == -1 || !S_ISREG(st.st_mode) || st.st_size <= 0 ||
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "Gateway request size limit policy must be a regular "
                       "file of 1..131072 bytes");
    return NGX_CONF_ERROR;
  }
  ...
  status = aurora_request_size_limit_create(
      bytes, used,
      (AuroraRequestSizeLimitEngine **)&conf->request_size_limit_engine);
  if (status != 0) {
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0,
        "invalid Gateway request size limit policy %V (status %ui)",
        &conf->request_size_limit_policy, (ngx_uint_t)status);
    return NGX_CONF_ERROR;
  }
...
ngx_int_t ngx_http_gateway_eval_request_size_limit(
  ...
  uint64_t header_bytes =
      (uint64_t)(r->request_length > 0 ? r->request_length : 0);
  uint64_t body_bytes = (uint64_t)(r->headers_in.content_length_n > 0
  ...
  uint32_t status = aurora_request_size_limit_evaluate(
      conf->request_size_limit_engine, host.data, host.len, r->uri.data,
      r->uri.len, client_ip.data, client_ip.len, header_bytes, body_bytes, r,
  ...
  if (status != 0) {
    ...
    ngx_log_error(NGX_LOG_ERR, &log, 0,
                  "Gateway request size limit eval failure: %ui",
                  (ngx_uint_t)status);
    return NGX_DECLINED;
  }
  ...
  if (decision.matched && !decision.allowed) {
    ...
    ngx_log_error(NGX_LOG_WARN, &log, 0,
                  "Gateway request size limit exceeded: rule %*s, status %ud, "
                  "headers %uL, body %uL",
                  (int)decision.rule_id_len, decision.rule_id,
                  (ngx_uint_t)decision.status_code, header_bytes, body_bytes);

    /* Prevent keep-alive socket reuse for oversized/rejected requests to avoid
     * pipeline poisoning */
    r->keepalive = 0;
