...
static void ngx_http_gateway_conn_limit_req_cleanup(void *data) {
...
static void ngx_http_gateway_conn_limit_cleanup(void *data) {
...
char *ngx_http_gateway_merge_conn_limit(ngx_conf_t *cf,
  ...
  ngx_conf_merge_str_value(conf->conn_limit_policy, prev->conn_limit_policy,
                           "");
  if (conf->conn_limit_policy.len == 0) {
    return NGX_CONF_OK;
  }
  if (((ngx_http_core_loc_conf_t *)ngx_http_conf_get_module_loc_conf(
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "Gateway connection limit requires satisfy all");
    return NGX_CONF_ERROR;
  }
  if (prev->conn_limit_engine &&
      conf->conn_limit_policy.data == prev->conn_limit_policy.data) {
    conf->conn_limit_engine = prev->conn_limit_engine;
    return NGX_CONF_OK;
  }
  if (ngx_conf_full_name(cf->cycle, &conf->conn_limit_policy, 0) != NGX_OK) {
    return NGX_CONF_ERROR;
  }
  ...
  if (fd == -1) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
                       "cannot open Gateway connection limit policy %V",
                       &conf->conn_limit_policy);
    return NGX_CONF_ERROR;
  }
  if (fstat(fd, &st) == -1 || !S_ISREG(st.st_mode) || st.st_size <= 0 ||
    ...
    close(fd);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "Gateway connection limit policy must be a regular file "
                       "of 1..131072 bytes");
    return NGX_CONF_ERROR;
  }
  ...
  if (status != 0) {
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0,
        "invalid Gateway connection limit policy %V (status %ui)",
        &conf->conn_limit_policy, (ngx_uint_t)status);
    return NGX_CONF_ERROR;
  }
...
uint32_t ngx_http_gateway_header_lookup(void *ctx, const uint8_t *name,
  ...
  if (r == NULL || name == NULL || name_len == 0 || out_val == NULL ||
      out_val_len == NULL) {
    return 1;
  }
  ...
  if (name_len == 13 &&
      ngx_strncasecmp((u_char *)name, (u_char *)"authorization", 13) == 0) {
...
ngx_int_t ngx_http_gateway_eval_conn_limit(ngx_http_request_t *r,
  ...
  uint32_t status = aurora_conn_limit_acquire(
      conf->conn_limit_engine, host.data, host.len, r->uri.data, r->uri.len,
  ...
  if (status == 1) {
    ...
    ngx_log_error(NGX_LOG_ERR, &log, 0,
                  "Gateway connection limit invalid request: %ui",
                  (ngx_uint_t)status);
    return NGX_HTTP_BAD_REQUEST;
  }
  if (status != 0) {
    ...
    log.handler = NULL;
    ngx_log_error(NGX_LOG_ERR, &log, 0,
                  "Gateway connection limit internal failure: %ui",
                  (ngx_uint_t)status);
  ...
  if (decision.action == 3) {
    ...
    for (h_idx = 0; h_idx < decision.headers_count &&
                    h_idx < AURORA_CONN_LIMIT_MAX_HEADERS;
         h_idx++) {
    ...
    /* Allowed: Attach cleanup to request pool to guarantee slot release on
     * finish or disconnect */
    ngx_pool_cleanup_t *cln = ngx_pool_cleanup_add(
        r->pool, sizeof(ngx_http_gateway_conn_limit_ctx_t));
    if (cln == NULL) {
