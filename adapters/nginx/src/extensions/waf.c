...
static void ngx_http_gateway_waf_cleanup(void *data) {
...
char *ngx_http_gateway_merge_waf(ngx_conf_t *cf, ngx_http_gateway_conf_t *prev,
   ...
   * nếu có module access khác cấu hình allow (ví dụ allow IP trong satisfy
   * any).
   */
  if (((ngx_http_core_loc_conf_t *)ngx_http_conf_get_module_loc_conf(
    ...
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0,
        "Gateway requires satisfy all to prevent access-phase bypass");
  ...
  if (conf->policy.len == 0) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "gateway on requires gateway_waf_policy");
    return NGX_CONF_ERROR;
  }
  ...
  if (ngx_conf_full_name(cf->cycle, &conf->policy, 0) != NGX_OK) {
    return NGX_CONF_ERROR;
  }
  ...
  if (fd == -1) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
                       "cannot open Gateway policy %V", &conf->policy);
    return NGX_CONF_ERROR;
  }
  ...
  if (fstat(fd, &st) == -1 || !S_ISREG(st.st_mode) || st.st_size <= 0 ||
    ...
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0,
        "Gateway policy must be a regular file of 1..65536 bytes");
    return NGX_CONF_ERROR;
  }
  ...
  if (status != 0) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "invalid Gateway policy %V (status %ui)", &conf->policy,
                       (ngx_uint_t)status);
    return NGX_CONF_ERROR;
  }
...
ngx_int_t ngx_http_gateway_eval_waf(ngx_http_request_t *r,
  ...
  uint32_t status = aurora_waf_evaluate_v4(conf->engine, host.data, host.len,
                                           r->uri.data, r->uri.len, &decision);
  if (status != 0 || decision.action > 1) {
    /*
     * Xử lý lỗi engine / panic:
     * Xóa log.handler tạm thời để tránh rò rỉ query/request nhạy cảm vào access
     * log NGINX.
     */
    ngx_log_t log = *r->connection->log;
    log.handler = NULL;
    ngx_log_error(NGX_LOG_ERR, &log, 0, "Gateway WAF evaluation failed: %ui",
                  (ngx_uint_t)status);
  ...
  if (decision.log_matches != 0 && gateway_log_count < 100) {
    ...
    ngx_log_error(NGX_LOG_INFO, &log, 0,
                  "Gateway match generation=%uL rule=%uL score=%ui logs=%ui",
                  decision.generation, decision.rule_id,
                  (ngx_uint_t)decision.score, (ngx_uint_t)decision.log_matches);
  }
  if (decision.action == 1) {
    ...
    if (conf->mode == 1) {
      ...
      ngx_log_error(NGX_LOG_NOTICE, &log, 0,
                    "Gateway audit: would block normalized path");
      return NGX_DECLINED;
    }
