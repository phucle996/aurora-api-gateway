...
static void ngx_http_gateway_jwt_cleanup(void *data) {
...
char *ngx_http_gateway_merge_jwt(ngx_conf_t *cf, ngx_http_gateway_conf_t *prev,
  ...
  if (((ngx_http_core_loc_conf_t *)ngx_http_conf_get_module_loc_conf(
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "Gateway JWT requires satisfy all");
  ...
  if (ngx_conf_full_name(cf->cycle, &conf->jwt_policy, 0) != NGX_OK) {
    return NGX_CONF_ERROR;
  }
  ...
  if (fd == -1) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
                       "cannot open Gateway JWT policy %V", &conf->jwt_policy);
    return NGX_CONF_ERROR;
  }
  if (fstat(fd, &st) == -1 || !S_ISREG(st.st_mode) || st.st_size <= 0 ||
    ...
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0,
        "Gateway JWT policy must be a regular file of 1..65536 bytes");
    return NGX_CONF_ERROR;
  }
  ...
  if (status != 0) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "invalid Gateway JWT policy %V (status %ui)",
                       &conf->jwt_policy, (ngx_uint_t)status);
    return NGX_CONF_ERROR;
  }
...
ngx_int_t ngx_http_gateway_eval_jwt(ngx_http_request_t *r,
  ...
  const u_char *authorization_data =
      authorization ? authorization->value.data : NULL;
  ...
  uint32_t status = aurora_jwt_evaluate(
  ...
  if (status == 1 || (status == 0 && decision.allowed == 0)) {
    extension_jwt_record_metrics(1);
  ...
  if (decision.headers_count > 0) {
    uint32_t count = decision.headers_count < AURORA_JWT_MAX_FORWARD_HEADERS
