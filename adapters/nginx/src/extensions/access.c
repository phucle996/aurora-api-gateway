...
static void ngx_http_gateway_access_cleanup(void *data) {
...
char *ngx_http_gateway_merge_access(ngx_conf_t *cf,
  ...
  if (((ngx_http_core_loc_conf_t *)ngx_http_conf_get_module_loc_conf(
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "Gateway access requires satisfy all");
    return NGX_CONF_ERROR;
  }
  if (prev->access_engine &&
      conf->access_policy.data == prev->access_policy.data) {
    conf->access_engine = prev->access_engine;
    return NGX_CONF_OK;
  }

  /* Chuẩn hóa đường dẫn đầy đủ của file policy */
  if (ngx_conf_full_name(cf->cycle, &conf->access_policy, 0) != NGX_OK) {
    return NGX_CONF_ERROR;
  }
  ...
  if (fd == -1) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
                       "cannot open Gateway access policy %V",
                       &conf->access_policy);
    return NGX_CONF_ERROR;
  }
  ...
  if (fstat(fd, &st) == -1 || !S_ISREG(st.st_mode) || st.st_size <= 0 ||
    ...
    ngx_conf_log_error(
        NGX_LOG_EMERG, cf, 0,
        "Gateway access policy must be a regular file of 1..65536 bytes");
    return NGX_CONF_ERROR;
  }
  ...
  if (status != 0) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "invalid Gateway access policy %V (status %ui)",
                       &conf->access_policy, (ngx_uint_t)status);
    return NGX_CONF_ERROR;
  }
...
ngx_int_t ngx_http_gateway_eval_access(ngx_http_request_t *r,
  ...
  uint32_t status =
      aurora_access_evaluate(conf->access_engine, &input, &decision);
  ...
  aurora_telemetry_record_access(0);
  return NGX_DECLINED;
}
