...
ngx_int_t
ngx_http_gateway_telemetry_zone_init(ngx_shm_zone_t *zone, void *previous)
{
...
static ngx_int_t
ngx_http_gateway_generation(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data)
{
...
/* Optional observability variables, not an all-workers activation acknowledgement. */
ngx_int_t
ngx_http_gateway_variables(ngx_conf_t *cf)
{
...
/*
 * Thiết lập location content handler khi directive gateway_metrics / aurora_waf_metrics xuất hiện.
 */
char *
ngx_http_gateway_metrics_directive(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
 ...
 */
ngx_int_t
ngx_http_gateway_metrics_handler(ngx_http_request_t *r)
{
    ngx_int_t                 rc;
    ngx_buf_t                *b;
    ngx_chain_t               out;
