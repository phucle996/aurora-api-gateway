use super::engine::{RequestMirrorEngine, RequestMirrorEvalRequest};
...
fn test_flat_config_evaluation() {
    ...
    let engine = RequestMirrorEngine::from_snapshot(json.as_bytes()).expect("parse flat config");
...
fn test_method_filtering() {
    ...
    let engine = RequestMirrorEngine::from_snapshot(json.as_bytes()).expect("parse methods rule");
...
fn test_sampling_rate() {
    ...
    let engine = RequestMirrorEngine::from_snapshot(json.as_bytes()).expect("parse sample rule");
